...
export const test = base.extend<ExtensionFixtures>({
  context: async ({ browserName }, use) => {
    ...
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Extension manifest not found at ${manifestPath}. Run 'npm run build:chromium' first.`);
    }
    ...
    if (browserName === 'chromium') {
      ...
      const userDataDir = path.join(__dirname, '../../../.test-user-data-firefox');
  ...
  extensionId: async ({ context }, use) => {
    ...
    try {
      ...
      console.log(`Service workers: ${workers.length}, Background pages: ${bgPages.length}`);
      ...
      await tempPage.goto('https://example.com', { waitUntil: 'domcontentloaded', timeout: 5000 }).catch(() => {});
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      if (!extensionId) {
        console.warn('⚠️  Extension ID not detected automatically. Some tests requiring the extension ID may be skipped.');
...
export async function setupMockAPIKeys(context: BrowserContext, extensionId?: string) {
  try {
    console.log('Setting up mock API keys...');
    
    // Get API key from environment variable
    const geminiApiKey = process.env.GEMINI_API_KEY || process.env.TEST_GEMINI_API_KEY;
    if (!geminiApiKey) {
      console.warn('⚠️  No GEMINI_API_KEY environment variable set. Set TEST_GEMINI_API_KEY or GEMINI_API_KEY to run e2e tests.');
      return;
    }
    ...
    if (extensionId) {
      ...
      }, geminiApiKey).catch(e => console.log('Page evaluate failed:', e));
