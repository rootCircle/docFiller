	...
	async fill(element, fieldType, fieldValue, value) {
		if (fieldType !== null) {
			switch (fieldType) {
				...
				case QType.DROPDOWN:
					return await this.fillDropDown(
						element,
						fieldValue,
						'Option 2',
						SLEEP_DURATION,
				...
				case QType.CHECKBOX_GRID:
					return await this.fillCheckboxGrid(
						fieldValue,
						[
				...
				case QType.MULTIPLE_CHOICE_GRID:
					return this.fillMultipleChoiceGrid(
						fieldValue,
						[
				...
				case QType.DATE_TIME_WITHOUT_YEAR:
					return this.fillDateTimeWithoutYear(
						fieldValue,
						'22-01-01-01',
						SLEEP_DURATION,
					);
		...
		fieldValue.options.forEach((element) => {
			value.forEach((val) => {
				...
				if (typeof val === 'string') {
					...
					fieldValue.other[0].click();
				} else if (
					typeof val === 'object' &&
					fieldValue.other !== undefined &&
					val.optionTitle === fieldValue?.other[0].data
				) {
